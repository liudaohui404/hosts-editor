  ...
  <Suspense>
    <div id="main" class="flex h-screen">
      ...
      <div class="w-80 p-3 flex flex-col bg-gray-50">
          ...
          <ul>
            ...
            <li>
              <button
                ...
                :class="{
                  'border-blue-500 bg-blue-50': currentEditingIndex === 0,
                }"
            ...
            <template v-for="(item, index) in groups" :key="item.label">
              <li>
                <button
                  class="w-full box-border py-1.5 px-3 rounded mt-2 flex justify-between items-center border transition-all duration-200"
                  :class="getGroupButtonClass(item, index)"
            ...
            <li v-if="isAddingNewGroup">
              <div
                class="w-full box-border py-1.5 px-3 rounded mt-2 border border-blue-300 bg-blue-50"
              >
                <div class="flex items-center">
                  <input
                    ref="newGroupInput"
                  ...
                  <div class="flex items-center space-x-1 ml-2">
                    <div class="flex items-center space-x-2 ml-2">
                      <button
                        type="button"
                        class="btn-action w-6 h-6 inline-flex items-center justify-center rounded-full bg-blue-500 text-white disabled:cursor-not-allowed"
                      ...
                      <button
                        type="button"
                        class="btn-action w-6 h-6 inline-flex items-center justify-center rounded-full bg-white border border-gray-200 text-gray-700  "
                        @mousedown.prevent
        ...
        <!-- 添加分组按钮 -->
        <div
          class="flex justify-center items-center sticky bottom-0 p-3 bg-white border-t border-gray-200"
        >
          <button
            class="w-full flex items-center justify-center gap-2 py-2 px-3 rounded border border-gray-200 bg-white text-sm text-gray-700 hover:bg-gray-50 transition-colors"
            @click="startAddGroup"
            title="添加分组"
            aria-label="添加分组"
          >
            <span class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-gray-100 text-gray-600">
              <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                <path d="M12 5v14M5 12h14" />
              </svg>
            </span>
            <span class="text-sm font-medium">添加分组</span>
          </button>
        </div>
      </div>
...
const showPermissionWarning = () => {
  ...
  if (isMac) {
    showMessage(
      "修改 hosts 文件需要管理员权限，系统将弹出密码输入对话框",
      "warning"
    );
  } else {
    showMessage(
      "修改 hosts 文件需要管理员权限，请以管理员身份运行此应用",
      "warning"
    );
  }
};
...
const refreshGroups = async () => {
  try {
    ...
    const localGroups: Group[] = localGroupsStr
      ? JSON.parse(localGroupsStr)
      : [];
    ...
    const hostsGroups = await Promise.all(
      groupNames.map(async (name) => {
        const entries: any[] = await invoke("get_hosts_group_entries", {
          groupName: name,
        });
        const hosts = entries
          .map(
...
const showHostsDetail = (index: number) => {
  ...
  if (editorContentChanged.value && currentEditingIndex.value > 0) {
    const dialogApi =
      internalInstance?.appContext.config.globalProperties.$dialog;
...
const deleteItem = async (index: number) => {
  ...
  try {
    ...
    if (
      ...
      showMessage(
        `分组 "${group.label}" 在 hosts 文件中未找到，已从本地删除`,
        "warning"
      );
      groups.value.splice(index, 1);
      localStorage.setItem("hosts-groups", JSON.stringify(groups.value));
...
// 获取分组按钮的样式类
const getGroupButtonClass = (group: Group, index: number) => {
  const isCurrentEditingGroup = currentEditingIndex.value === index + 1;
  const isActive = group.active;

  // 当前选中状态 > 激活状态 > 未激活状态
  if (isCurrentEditingGroup) {
    return "border-blue-500 bg-blue-50 shadow-sm ring-1 ring-blue-200";
  } else if (isActive) {
    return "border-green-500 bg-green-50 hover:bg-green-100 hover:shadow-sm";
  } else {
    return "border-gray-300 bg-white hover:bg-gray-50 hover:border-gray-400";
  }
};
...
onMounted(async () => {
  try {
    ...
    editor.value = new EditorView({
      ...
      doc: sysHosts,
      parent: editorArea.value?.querySelector(
        ".editor-container"
      ) as HTMLElement,
    });
